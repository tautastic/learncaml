...
let check_ok (start, stop, step) expected =
  ...
  | Ok lst ->
      check (list char) (Printf.sprintf "%c %c %d" start stop step) expected lst
...
let check_error (start, stop, step) expected_msg =
  ...
  | Error e ->
      check string (Printf.sprintf "%c %c %d" start stop step) expected_msg e
...
let test_alpha_range () =
  ...
  check_error ('G', 'V', 0)
    "step must be a non-zero value between -26 and 26, exclusive";
  check_error ('N', 'I', -30)
    "step must be a non-zero value between -26 and 26, exclusive";
  check_error ('n', 'c', -29)
    "step must be a non-zero value between -26 and 26, exclusive";
